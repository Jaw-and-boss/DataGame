        ...
        void DrawMenu()
        {
            ...
            if (!TC_Settings.instance.hideMenuBar)
            {
                ...
                if (Application.isPlaying) 
                {
                    ...
                    GUI.Button(new Rect(300, 2, 350, 20), "Close the TC2 window to measure real performance", EditorStyles.miniButtonMid);
                    GUI.color = Color.white;
                }

                GUILayout.Space(window.position.width - 620 - ((width * 2) + 45));
            }
            else GUILayout.Space(window.position.width - 449 - (width + 5));
